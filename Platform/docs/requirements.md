...
Produce a DocFX conceptual article that documents the Operations.Extensions project, (go over every C# code file).
...
Your article must adhere to the following standards summarized below.
...
-   **Sentence‐style headings** – Capitalize only the first word and proper nouns; never add end punctuation.
...
-   Use active voice unless actor is irrelevant.
...
Use Microsoft alert blocks sparingly—never stack two together:
...
1.  **Build-ready content** – Ensure all code snippet references are valid and all Markdown syntax is correct.
...
Do not worry about the existing documentation format; if it does not adhere to the standards defined here, update it.
