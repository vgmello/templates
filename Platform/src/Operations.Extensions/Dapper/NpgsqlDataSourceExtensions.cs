...
public static class NpgsqlDataSourceExtensions
{
    ...
    /// <param name="paramsProvider">Optional provider for command parameters.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A sequence of data of the T type; if a basic type (int, string, etc.) is queried then the data from the first column in assumed, otherwise an instance is created per row, and a direct column-name===member-name mapping is assumed (case insensitive).</returns>
    ...
    /// <param name="paramsProvider">Optional provider for command parameters.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A single row of data of the T type; if a basic type (int, string, etc.) is queried then the data from the first column in assumed, otherwise an instance is created per row, and a direct column-name===member-name mapping is assumed (case insensitive).</returns>
    ...
    /// <summary>
    /// Executes a query and returns a single row, or a default value if the sequence is empty. Throws an exception if there is more than one element.
    /// </summary>
    /// <typeparam name="T">The type of result to return.</typeparam>
    /// <param name="dataSource">The Npgsql data source.</param>
    /// <param name="commandProvider">Provider for command name and type.</param>
    ...
    /// <param name="paramsProvider">Optional provider for command parameters.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The first row of data of the T type; if a basic type (int, string, etc.) is queried then the data from the first column in assumed, otherwise an instance is created per row, and a direct column-name===member-name mapping is assumed (case insensitive).</returns>
