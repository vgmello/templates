...
public class TestProgram
{
    public static void Main()
    {
        ...
        Console.WriteLine("Comparing implementations:");
        ...
        foreach (var input in testInputs)
        {
            ...
            var optimized = ToLowerCaseWithSeparator_Optimized(input, '_');
            var match = current == optimized;
            allMatch &= match;

            Console.WriteLine($"{input,-30} {current,-30} {optimized,-30} {(match ? "✓" : "✗")}");
        }

        Console.WriteLine($"\nAll tests passed: {(allMatch ? "YES" : "NO")}");
    }
