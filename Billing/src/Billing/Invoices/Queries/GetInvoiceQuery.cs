...
{
    public static async Task<InvoiceModel> Handle(GetInvoiceQuery query,
        NpgsqlDataSource dataSource, CancellationToken cancellationToken)
    {
        ...
        var invoice = await connection.QuerySingleOrDefaultAsync<InvoiceModel>(sql, new { Id = query.Id });
