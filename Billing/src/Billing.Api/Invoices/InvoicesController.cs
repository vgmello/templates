...
public class InvoicesController(IMessageBus bus) : ControllerBase
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<InvoiceModel>> GetInvoice(
        [FromRoute] Guid id,
        CancellationToken cancellationToken = default)
    {
        try
    ...
    [HttpPost]
    public async Task<ActionResult<InvoiceModel>> CreateInvoice(
    ...
    public async Task<ActionResult<InvoiceModel>> CancelInvoice(
        Guid id,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<ActionResult<InvoiceModel>> MarkInvoiceAsPaid(
        Guid id,
        [FromBody] MarkInvoiceAsPaidRequest request,
        CancellationToken cancellationToken = default)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return commandResult.Match<ActionResult>(
            success => Ok(new { Message = "Payment simulation triggered successfully" }),
            errors => BadRequest(new { Errors = errors }));
    }
}
