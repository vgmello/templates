import * as grpc from '@grpc/grpc-js';
import * as protoLoader from '@grpc/proto-loader';
import { env } from '$env/dynamic/private';
import { dev } from '$app/environment';

// Configuration
const GRPC_HOST = env.GRPC_HOST || 'localhost';
const GRPC_PORT = env.GRPC_PORT || '5061';
const PROTO_PATH = env.PROTO_PATH || './protos/billing.proto';

// Error class for gRPC errors
class GrpcError extends Error {
	constructor(message, code, details) {
		super(message);
		this.name = 'GrpcError';
		this.code = code;
		this.details = details;
	}
}

let client = null;

/**
 * Initialize gRPC client
 */
async function initializeClient() {
	if (client) return client;

	try {
		// In development, we might not have the proto file, so we'll mock it
		if (dev && !env.PROTO_PATH) {
			console.warn('gRPC proto path not configured, using mock client');
			return createMockClient();
		}

		// Load proto definition
		const packageDefinition = protoLoader.loadSync(PROTO_PATH, {
			keepCase: true,
			longs: String,
			enums: String,
			defaults: true,
			oneofs: true,
		});

		const protoDescriptor = grpc.loadPackageDefinition(packageDefinition);
		const BillingService = protoDescriptor.billing.BillingService;

		// Create client
		client = new BillingService(
			`${GRPC_HOST}:${GRPC_PORT}`,
			grpc.credentials.createInsecure()
		);

		return client;
	} catch (error) {
		console.error('Failed to initialize gRPC client:', error);
		
		// Fallback to mock client in development
		if (dev) {
			console.warn('Falling back to mock gRPC client');
			return createMockClient();
		}
		
		throw new GrpcError('Failed to initialize gRPC client', grpc.status.UNAVAILABLE, error.message);
	}
}

/**
 * Create a mock client for development/testing
 */
function createMockClient() {
	const mockCashiers = [
		{
			cashierId: "a52757cd-a42f-4fb9-8566-a98c61a71d2a",
			name: "Test Cashier",
			email: "test@example.com",
			cashierPayments: [
				{ currency: "USD", isActive: true, createdDateUtc: new Date().toISOString() },
				{ currency: "EUR", isActive: true, createdDateUtc: new Date().toISOString() }
			],
			createdDateUtc: new Date().toISOString(),
			updatedDateUtc: new Date().toISOString(),
			version: 1
		},
		{
			cashierId: "b52757cd-a42f-4fb9-8566-a98c61a71d2a",
			name: "John Doe", 
			email: "john.doe@example.com",
			cashierPayments: [
				{ currency: "USD", isActive: true, createdDateUtc: new Date().toISOString() }
			],
			createdDateUtc: new Date().toISOString(),
			updatedDateUtc: new Date().toISOString(),
			version: 1
		}
	];

	return {
		getCashiers: (request, callback) => {
			setTimeout(() => {
				callback(null, { cashiers: mockCashiers });
			}, 100);
		},
		
		getCashier: (request, callback) => {
			setTimeout(() => {
				const cashier = mockCashiers.find(c => c.cashierId === request.cashierId);
				if (cashier) {
					callback(null, cashier);
				} else {
					callback({
						code: grpc.status.NOT_FOUND,
						message: 'Cashier not found'
					});
				}
			}, 100);
		},
		
		createCashier: (request, callback) => {
			setTimeout(() => {
				const newCashier = {
					cashierId: crypto.randomUUID(),
					name: request.name,
					email: request.email,
					cashierPayments: request.currencies?.map(currency => ({
						currency,
						isActive: true,
						createdDateUtc: new Date().toISOString()
					})) || [],
					createdDateUtc: new Date().toISOString(),
					updatedDateUtc: new Date().toISOString(),
					version: 1
				};
				mockCashiers.push(newCashier);
				callback(null, newCashier);
			}, 200);
		},
		
		updateCashier: (request, callback) => {
			setTimeout(() => {
				const index = mockCashiers.findIndex(c => c.cashierId === request.cashierId);
				if (index !== -1) {
					const updatedCashier = {
						...mockCashiers[index],
						...request,
						updatedDateUtc: new Date().toISOString(),
						version: mockCashiers[index].version + 1
					};
					mockCashiers[index] = updatedCashier;
					callback(null, updatedCashier);
				} else {
					callback({
						code: grpc.status.NOT_FOUND,
						message: 'Cashier not found'
					});
				}
			}, 200);
		},
		
		deleteCashier: (request, callback) => {
			setTimeout(() => {
				const index = mockCashiers.findIndex(c => c.cashierId === request.cashierId);
				if (index !== -1) {
					mockCashiers.splice(index, 1);
					callback(null, {});
				} else {
					callback({
						code: grpc.status.NOT_FOUND,
						message: 'Cashier not found'
					});
				}
			}, 200);
		}
	};
}

/**
 * Promisify gRPC call
 */
function promisifyCall(client, methodName, request) {
	return new Promise((resolve, reject) => {
		client[methodName](request, (error, response) => {
			if (error) {
				reject(new GrpcError(
					error.message || 'gRPC call failed',
					error.code || grpc.status.UNKNOWN,
					error.details
				));
			} else {
				resolve(response);
			}
		});
	});
}

/**
 * Cashier service using gRPC
 */
export const cashierGrpcService = {
	async getCashiers() {
		const client = await initializeClient();
		const response = await promisifyCall(client, 'getCashiers', {});
		return response.cashiers || [];
	},

	async getCashier(cashierId) {
		const client = await initializeClient();
		return await promisifyCall(client, 'getCashier', { cashierId });
	},

	async createCashier(cashierData) {
		const client = await initializeClient();
		const request = {
			name: cashierData.name,
			email: cashierData.email,
			currencies: cashierData.currencies
		};
		return await promisifyCall(client, 'createCashier', request);
	},

	async updateCashier(cashierId, cashierData) {
		const client = await initializeClient();
		const request = {
			cashierId,
			...cashierData
		};
		return await promisifyCall(client, 'updateCashier', request);
	},

	async deleteCashier(cashierId) {
		const client = await initializeClient();
		return await promisifyCall(client, 'deleteCashier', { cashierId });
	}
};

/**
 * Close gRPC client (for cleanup)
 */
export function closeGrpcClient() {
	if (client && typeof client.close === 'function') {
		client.close();
		client = null;
	}
}